...
import { Trophy, Settings, LogOut, Star, Palette, HelpCircle, Sun, Moon, Shield, Activity, Zap, TrendingUp, Award } from 'lucide-react';
  ...
  useEffect(() => {
    const getUserData = async () => {
      try {
        ...
        // Simulation de données de progression basées sur l'historique
        if (payments) {
          let currentPoints = 0;
          const chartData = payments.map((p, i) => {
            currentPoints += 10; // +10 par participation
            return {
              name: `T${i+1}`,
